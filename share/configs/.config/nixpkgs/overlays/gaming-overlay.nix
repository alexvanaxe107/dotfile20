self: super:
{
  ...
  gamingPackages = super.gamingPackages or {} // {
   ...
   gamescope = self.gamescope;
    ...
    nix-rebuild = super.writeScriptBin "nix-rebuild" ''
      ...
      if ! command -v nix-env &>/dev/null; then
          echo "warning: nix-env was not found in PATH, add nix to gamingPackages" >&2
          PATH=${self.nix}/bin:$PATH
      fi
      exec nix-env -f '<nixpkgs>' -r -iA gamingPackages "$@"
    '';
